...
pub fn initialize_from_wal(wal_file_path: &str, txd_count: Arc<RwLock<u32>>, vid_count: Arc<RwLock<u32>>, current_transaction: Arc<RwLock<Transaction>>, file: Arc<RwLock<File>>, new_node: Arc<RwLock<Node>>, all_addr: Arc<RwLock<Vec<SocketAddr>>>, ts_ord: Arc<RwLock<HashMap<u32, u32>>>) {
            ...
            for items in value.lines() {
                ...
                if load_to_cli {
                    for vals in uncommitted_strings.iter() {
                        match cli(vals.clone(), Arc::clone(&txd_count), Arc::clone(&vid_count), Arc::clone(&current_transaction), Arc::clone(&file), Arc::clone(&new_node), None, Arc::clone(&all_addr), Arc::clone(&ts_ord)) {
                            Ok(_) => {}
